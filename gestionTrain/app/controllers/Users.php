    ...
    class Users extends Controller 
    {
        ...
        public function register()
        {
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST")
            {
                ...
                //Validate full name
                }elseif(!preg_match($fullNameValidation, $data["fullName"])){
                    $data["fullNameError"] = 'Full name can only contain letters.';
                    // Validate email 
                }elseif (!filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
                    $data['emailError'] = 'Please enter the correct format.';
                //Check if email exists , Check if username exists.
                } elseif ($this->user->findUserByEmail($data['email'])) {
                    $data['emailError'] = 'Email is already taken.';
                }elseif($this->client->findClientByUsername($data['username'])){
                ...
                if (empty($data['Errors']) && empty($data['usernameError']) && empty($data["fullNameError"]) && 
        ...
        public function profil()
        {
            ...
            if(is_logged_user()){
                ...
                if($_SERVER["REQUEST_METHOD"] == "POST")
                {
                    ...
                    if(empty($data['username']) || empty($data["fullName"]) || empty($data["phone"]) || empty($data["email"])) 
                    {
                        $data["Errors"] = "Some field is empty, Try again!";
